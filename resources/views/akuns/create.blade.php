...
@section('container')
    ...
    <div class="col-lg-8">
        <form method="POST" action="/akuns" class="mb-5" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
                <label for="nama_akun" class="form-label">Nama Akun</label>
                <input type="text" class="form-control @error('nama_akun') is-invalid @enderror" id="nama_akun"
                    name="nama_akun" value="{{ old('nama_akun') }}">
                @error('nama_akun')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('no_akun') is-invalid @enderror" id="no_akun"
                    name="no_akun" value="{{ old('no_akun') }}">
                @error('no_akun')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="sub_sub_kode_rekening_id">
                    @foreach ($subsubkoderekenings as $subsubkoderekening)
                        <option value="{{ $subsubkoderekening->id }}" @if (old('sub_sub_kode_rekening_id') == $subsubkoderekening->id) selected @endif>
